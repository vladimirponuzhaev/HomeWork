...
void PrintResult(double[] array)
{
    ...
    {
        // Write(Math.Round(array[i], 2).ToString() + ", "); // В таком виде на получается сообразить, как вывести элементы массива через "," с "." в конце.
        Write(array[i].ToString("0.00") + ", "); // В таком виде на получается сообразить, как вывести элементы массива через "," с "." в конце.
    }
    // WriteLine(String.Join(", ", array) + "."); // В таком виде на получается сообразить, как вывести элементы массива с округлением до .00.
    WriteLine();
